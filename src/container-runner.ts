...
async function buildContainerArgs(
  ...
  if (process.env.CLAUDE_CODE_USE_VERTEX === '1') {
    ...
    if (process.env.ANTHROPIC_VERTEX_PROJECT_ID)
      args.push('-e', 'ANTHROPIC_VERTEX_PROJECT_ID=' + process.env.ANTHROPIC_VERTEX_PROJECT_ID);
  ...
  // configured, treated as a transient hard failure: if we can't wire
  // the gateway, we don't spawn. The caller (router or host-sweep)
  // catches the throw, leaves the inbound message pending, and the next
  // sweep tick retries. Skipped entirely when ONECLI_API_KEY isn't set;
  // without a key the SDK 401s on every ensureAgent call against the
  // public app.onecli.sh fallback URL, so deployments that don't use
  // OneCLI's vault (TRMM creds live in the approval-bridge, not in
  // containers) need this skip path.
  if (ONECLI_API_KEY) {
    if (agentIdentifier) {
